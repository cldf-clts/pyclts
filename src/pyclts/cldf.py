"""
This module specifies the three data tables of the CLTS CLDF dataset which are created from the
pkg data when running `clts dist`.
"""
import dataclasses
...
class FeatureTable(CLDFTable):
    ...
    this table.
    """
    ID: str = dataclasses.field(  # pylint: disable=C0103
        metadata={"propertyUrl": "http://cldf.clld.org/v1.0/terms.rdf#id"}
    )
    TYPE: BaseSoundclassType = dataclasses.field(  # pylint: disable=C0103
        metadata={
            "dc:description":
    ...
    FEATURE: str = dataclasses.field(  # pylint: disable=C0103
        metadata={"dc:description": "Note that CLTS features are not necessarily binary."}
    )
    VALUE: str = dataclasses.field(  # pylint: disable=C0103
        metadata={}
    )
    ...
    @classmethod
    def iter_rows(cls, api, _) -> Generator[list[str], None, None]:
        """
...
@dataclasses.dataclass(frozen=True)
class SoundTable(CLDFTable):  # pylint: disable=R0902
    """
    This table lists sounds described with unique sets of feature values.
    """
    ID: str = dataclasses.field(  # pylint: disable=C0103
    ...
    NOTE: str = dataclasses.field(  # pylint: disable=C0103
        default='',
    ...
    def _get_features(sound, fids, log):
        ...
        for sound_ in csounds:
            for kk, vv in sound_.featuredict.items():
                if vv:
                    fid = f'{sound_.type()}_{kk}_{vv}'
                    if fid in fids:
                        features.append(fid)
                    else:
    ...
    def from_grapheme_and_sound(cls, grapheme, sound, fids, log):
        """
        Create an instance described by a grapheme.
        """
        return cls(
    ...
    def from_name_and_sound(cls, name, sound, fids, log):
        """
        Create an instance described by a name, i.e. feature vector.
        """
        return cls(
            ID=name.replace(' ', '_'),
            NAME=name,
...
@dataclasses.dataclass(frozen=True)
class GraphemeTable(CLDFTable):  # pylint: disable=R0902
    """
    Graphemes are symbols, i.e. strings, used to denote sounds in transcription systems.
    """
    PK: str = dataclasses.field(metadata={  # pylint: disable=C0103
        "propertyUrl": "http://cldf.clld.org/v1.0/terms.rdf#id"})
    GRAPHEME: str = dataclasses.field(metadata={  # pylint: disable=C0103
        "dc:description": "Grapheme used in a particular transcription to denote a sound"})
    NAME: str = dataclasses.field(metadata={  # pylint: disable=C0103
        "fk": SoundTable,
        "dc:description": "The ordered concatenation of feature values of the denoted sound"})
    EXPLICIT: str = dataclasses.field(metadata={  # pylint: disable=C0103
        "dc:description":
